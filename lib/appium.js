  ...
  async createSession (jsonwpCaps, reqCaps, w3cCapabilities) {
    ...
    try {
      ...
      if (this.args.relaxedSecurityEnabled) {
        log.info(`Applying relaxed security to '${InnerDriver.name}' as per server command line argument. This flag is DEPRECATED. Use --allow-insecure instead.`);
        d.relaxedSecurityEnabled = true;
      } else if (this.args.allowInsecure && this.args.allowInsecure.length > 0) {
        log.info(`Enabling insecure features ${JSON.stringify(this.args.allowInsecure)} for session`);
