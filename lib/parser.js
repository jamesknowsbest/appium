...
const args = [
  ...
  [['--allow-insecure'], {
    dest: 'allowInsecure',
    defaultValue: [],
    type: parseAllowInsecureFeatures,
    required: false,
    example: 'execute-webdriver,adb-shell',
...
const deprecatedArgs = [
  [['--relaxed-security'], {
    ...
    action: 'storeTrue',
    required: false,
    help: '[DEPRECATED] (Use --allow-insecure instead). ' +
          'Disable additional security checks, so it is possible to use some advanced features, provided ' +
...
function parseAllowInsecureFeatures (features) {
  try {
    return features.toString().split(',').map(s => s.trim()).filter(Boolean);
  } catch (err) {
    throw new Error('Could not parse value of --allow-insecure. Should be a list of strings separated by commas');
  }
}
